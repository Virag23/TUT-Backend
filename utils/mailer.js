...
const FROM          = `"Tirupati Road Lines Pvt. Ltd." <${process.env.MAIL_USER}>`;
...
const footer = `
  <div style="background:#080810;padding:24px 40px;border-top:1px solid #1e1e32;text-align:center;">
    <p style="font-family:Arial,sans-serif;font-size:13px;margin:0 0 5px;">
      ...
      <a href="mailto:tirupatiunion@gmail.com" style="color:#c9a227;text-decoration:none;"> tirupatiunion@gmail.com</a>
    </p>
    <p style="font-family:Arial,sans-serif;font-size:13px;margin:0 0 5px;">
      <span style="color:#f5f5f0;font-weight:600;">Phone 1:</span>
      <a href="tel:+918446123777" style="color:#c9a227;text-decoration:none;"> +91 8446123777</a>
    </p>
    <p style="font-family:Arial,sans-serif;font-size:13px;margin:0 0 5px;">
      <span style="color:#f5f5f0;font-weight:600;">Phone 2:</span>
      <a href="tel:+919371237770" style="color:#c9a227;text-decoration:none;"> +91 9371237770</a>
    </p>
...
export async function sendInquiryEmails({ senderName, email, phone, subject, message }) {
  ...
  try {
    ...
    console.log('✅ Company inquiry alert sent');
  } catch (err) {
    console.error('❌ Company inquiry email failed:', err.message);
  }
  ...
  if (email) {
    try {
        ...
        html: wrap(`
          ...
          <div style="background:#111827;border:1px solid rgba(201,162,39,0.25);border-radius:8px;padding:20px 24px;">
            ...
            <p style="font-family:Arial,sans-serif;color:#aaa;font-size:13px;line-height:1.8;margin:0;">
              A member of our team will personally reach out to address your query.
              In the meantime, feel free to call us at
              <a href="tel:+918446123777" style="color:#c9a227;text-decoration:none;font-weight:700;">+91 8446123777</a>.
            </p>
          </div>
        `),
      });
      ...
      console.error('❌ User inquiry email failed:', err.message);
    }
  }
}
...
export async function sendBookingEmails({ customerName, email, phone, materialType, weight, pickupLocation, dropLocation, date }) {
  ...
  try {
    await sendEmail({
      ...
      html: wrap(`
        ...
        <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
          ...
          ${row('Phone',    `<a href="tel:${phone}" style="color:#c9a227;text-decoration:none;font-weight:700;">${phone}</a>`)}
    ...
    console.log('✅ Company booking alert sent');
  } catch (err) {
    console.error('❌ Company booking email failed:', err.message);
  }
  ...
  if (email) {
    try {
      ...
      console.error('❌ User booking email failed:', err.message);
    }
  }
}
